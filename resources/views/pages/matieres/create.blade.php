...
@section('content')
    ...
    <form class="container mt-4" method="POST" action="{{ route('matieres.store') }}">
        @csrf
        <div class="row mb-3">
            <div class="col-md-12">
                <label for="nom_matiere" class="form-label">Nom de la matière</label>
                <input type="text" class="form-control" name="nom_matiere" id="nom_matiere" value="{{ old('nom_matiere') }}" required>
            </div>
        </div>
