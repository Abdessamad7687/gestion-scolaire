...
@section('content')
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Créer un groupe</li>
        </ol>
    </nav>
    ...
    <form class="container mt-4" method="POST" action="{{ route('groupes.store') }}">
        @csrf
        <div class="row mb-3">
            <div class="col-md-6">
                <label for="max_etudiants" class="form-label">Nombre maximum d'étudiants</label>
                <input type="number" class="form-control" name="max_etudiants" id="max_etudiants" min="1" value="{{ old('max_etudiants') }}" required>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <select class="form-select form-control" name="filiere_id" id="filiere_id" required>
                    ...
                    @foreach ($filieres as $filiere)
                        <option value="{{ $filiere->id }}" @selected(old('filiere_id') == $filiere->id)>{{ $filiere->nom_filiere }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6">
                ...
                <select class="form-select form-control" name="niveau_id" id="niveau_id" required>
                    ...
                    @foreach ($niveaux as $niveau)
                        <option value="{{ $niveau->id }}" @selected(old('niveau_id') == $niveau->id)>{{ $niveau->nom_niveau }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="col-md-6">
                ...
                <select class="form-select form-control" name="matiere_id" id="matiere_id" required>
                    ...
                    @foreach ($matieres as $matiere)
                        <option value="{{ $matiere->id }}" @selected(old('matiere_id') == $matiere->id)>{{ $matiere->nom_matiere }}</option>
                    @endforeach
                </select>
            </div>
        </div>
