...
@section('content')
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Créer une comission</li>
        </ol>
    </nav>
    ...
    <form class="container mt-4" method="POST" action="{{ route('comissions.store') }}">
        @csrf
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <select class="form-select form-control" name="professeur_id" id="professeur_id" required>
                    ...
                    @foreach ($professeurs as $professeur)
                        <option value="{{ $professeur->id }}" @selected(old('professeur_id') == $professeur->id)>{{ $professeur->nom }} {{ $professeur->prenom }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6">
                ...
                <select class="form-select form-control" name="etudiant_id" id="etudiant_id" required>
                    ...
                    @foreach ($etudiants as $etudiant)
                        <option value="{{ $etudiant->id }}" @selected(old('etudiant_id') == $etudiant->id)>{{ $etudiant->nom }} {{ $etudiant->prenom }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" min="0" class="form-control" name="montant" id="montant" value="{{ old('montant') }}" required>
            </div>
            <div class="col-md-4">
                <label for="datecomission" class="form-label">Date</label>
                <input type="date" class="form-control" name="datecomission" id="datecomission" value="{{ old('datecomission') }}" required>
            </div>
            <div class="col-md-4">
                ...
                <select class="form-select form-control" name="statutcomission" id="statutcomission" required>
                    ...
                    @foreach (['Payée', 'En attente', 'Annulée'] as $statut)
                        <option value="{{ $statut }}" @selected(old('statutcomission') == $statut)>{{ $statut }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-md-12 d-flex justify-content-end">
                <button type="submit" class="btn btn-primary">Créer la comission</button>
            </div>
        </div>
    </form>
@endsection
