...
@section('content')
    <div class="container">
        <div class="row col-md-12 d-flex justify-content-between">
            ...
            <div class="col-md-6">
                <a href="{{ route('paiements.create') }}" class="btn btn-primary d-flex float-right">
    ...
    <div style="overflow-x: auto; max-width: 100%;">
        <table id="paiements-table" class="table table-striped" style="width:100%;">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($paiements as $paiement)
                    <tr>
                        <td>{{ $paiement['id'] }}</td>
                        <td>{{ $paiement['etudiant'] ?? '—' }}</td>
                        <td>{{ number_format($paiement['montant'], 2, ',', ' ') }} MAD</td>
                        <td>{{ $paiement['pourcentage'] }}%</td>
                        <td>{{ $paiement['date'] }}</td>
                        <td>
                            @php
                            ...
                            @endphp
                            <span class="badge {{ $badgeClass }}">{{ $paiement['statut'] }}</span>
                        </td>
                        <td>
                            <div class="row">
