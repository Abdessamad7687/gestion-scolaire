...
@section('content')
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Modifier un paiement</li>
        </ol>
    </nav>
    ...
    <form class="container mt-4" method="POST" action="{{ route('paiements.update', $paiement->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="row mb-3">
                ...
                <select class="form-select form-control" name="etudiant_id" id="etudiant_id" required>
                    ...
                    @foreach ($etudiants as $etudiant)
                        <option value="{{ $etudiant->id }}"
                            @selected(old('etudiant_id', $paiement->etudiant_id) == $etudiant->id)>
            ...
            <div class="col-md-3">
                ...
                <input type="number" step="0.01" min="0" max="100" class="form-control" name="pourcentage"
                    id="pourcentage" value="{{ old('pourcentage', $paiement->pourcentage) }}" required>
            </div>
        </div>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select form-control" name="statutpaiement" id="statutpaiement" required>
                    ...
                    @foreach (['Payé', 'En attente', 'Annulé'] as $statut)
                        <option value="{{ $statut }}"
                            @selected(old('statutpaiement', $paiement->statutpaiement) == $statut)>{{ $statut }}</option>
                    @endforeach
                </select>
            </div>
        </div>
